...
private:
  ::std::unique_ptr<::qlz_state_compress> compress;
  int const compSize;
  int compIndex=0;
  ::std::unique_ptr<char[]> compBuf;

  ::std::unique_ptr<::qlz_state_decompress> decomp;
  int bytesRead=0;
  char* compressedStart;

  bool doFlush (::sockaddr* addr,::socklen_t len){
    int const bytes=sockWrite(sock_,compBuf.get(),compIndex,addr,len);
    if(bytes==compIndex){compIndex=0;return true;}
    compIndex-=bytes;
    ::memmove(compBuf.get(),compBuf.get()+bytes,compIndex);
    return false;
  }

public:
  explicit BufferCompressed (int sz):SendBufferHeap(sz),ReceiveBuffer<R>(new char[sz],0)
  ...
  bool Flush (::sockaddr* addr=nullptr,::socklen_t len=0){
    ...
    if(index>0){
      ...
      compIndex+=::qlz_compress(buf,compBuf.get()+compIndex,index,compress.get());
      Reset();
  ...
  bool GotPacket (){
    ...
    try{
      ...
      if(kosher){
        ...
        ::qlz_decompress(compressedStart,rbuf,decomp.get());
        bytesRead=0;
        this->Update();
        return true;
      }else{
